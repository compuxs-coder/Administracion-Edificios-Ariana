...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::unprepared($this->occupancyGuardSql($schema, true));

        DB::unprepared(<<<SQL
            CREATE FUNCTION "{$schema}"."verify_owner_occupancy_history"()
            RETURNS trigger LANGUAGE plpgsql AS \$\$
            BEGIN
                IF EXISTS (
                    SELECT 1
                    FROM "{$schema}"."departamento_residentes" occupation
                    JOIN "{$schema}"."residentes" resident ON resident.id = occupation.residente_id
                    WHERE occupation.tipo_ocupacion = 'propietario_ocupante'
                      AND NOT EXISTS (
                          SELECT 1
                          FROM "{$schema}"."propietarios" owner_profile
                          JOIN "{$schema}"."departamento_propietarios" ownership
                            ON ownership.propietario_id = owner_profile.id
                          WHERE owner_profile.tercero_id = resident.tercero_id
                            AND ownership.departamento_id = occupation.departamento_id
                            AND ownership.fecha_inicio <= occupation.fecha_inicio
                            AND (ownership.fecha_fin IS NULL OR ownership.fecha_fin > occupation.fecha_inicio)
                      )
                ) THEN
                    RAISE EXCEPTION 'Una ocupación de propietario requiere titularidad vigente al inicio.' USING ERRCODE = '23514';
                END IF;
            ...
            CREATE CONSTRAINT TRIGGER ocupaciones_propietario_correspondencia_guard
            AFTER INSERT OR UPDATE
            ON "{$schema}"."departamento_residentes"
            DEFERRABLE INITIALLY DEFERRED
            FOR EACH ROW
            EXECUTE FUNCTION "{$schema}"."verify_owner_occupancy_history"();

            CREATE CONSTRAINT TRIGGER titularidades_ocupacion_correspondencia_guard
    ...
    public function down(): void
    {
        ...
        DB::unprepared(<<<SQL
            DROP TRIGGER IF EXISTS titularidades_ocupacion_correspondencia_guard
            ON "{$schema}"."departamento_propietarios";
            DROP TRIGGER IF EXISTS ocupaciones_propietario_correspondencia_guard
            ON "{$schema}"."departamento_residentes";
            DROP FUNCTION IF EXISTS "{$schema}"."verify_owner_occupancy_history"();
            SQL);

        DB::unprepared($this->occupancyGuardSql($schema, false));
    }

    private function occupancyGuardSql(string $schema, bool $insertOnlyOwnerCheck): string
    {
        $ownerCheck = $insertOnlyOwnerCheck ? "TG_OP = 'INSERT' AND " : '';
        ...
        return <<<SQL
            ...
            BEGIN
                ...
                IF {$ownerCheck}NEW.tipo_ocupacion = 'propietario_ocupante' AND NOT EXISTS (
                    SELECT 1
