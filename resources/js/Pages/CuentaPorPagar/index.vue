...
const providerItems = computed(() => {
  ...
  return unique.map(item => ({ label: item.nombre, value: item.id }))
})
const daysOverdue = (date: string) => Math.max(0, Math.floor((Date.now() - new Date(`${date}T00:00:00`).getTime()) / 86400000))
const accountState = (account: CuentaPorPagar): EstadoCuentaPorPagar => {
  if (account.estado === 'anulada') return 'anulada'
  if (account.estado === 'pagada' || Number(account.saldo) <= 0) return 'pagada'
  if (account.estado === 'parcial' || Number(account.saldo) < Number(account.montoOriginal)) return 'parcial'
  return 'pendiente'
}
const isOpen = (account: CuentaPorPagar) => accountState(account) !== 'anulada' && accountState(account) !== 'pagada' && Number(account.saldo) > 0
const isOverdue = (account: CuentaPorPagar) => isOpen(account) && daysOverdue(account.fechaVencimiento) > 0
const canRegister = (account: CuentaPorPagar) => canAny('desembolsos.registrar') && can('desembolsos.registrar', account.edificioId) && isOpen(account)
const summary = computed<CuentasPorPagarResumen>(() => props.resumen ?? {
  totalPendiente: props.cuentasPorPagar.data.filter(isOpen).reduce((total, item) => total + Number(item.saldo), 0).toFixed(4),
  totalVencido: props.cuentasPorPagar.data.filter(isOverdue).reduce((total, item) => total + Number(item.saldo), 0).toFixed(4),
  porVencer: props.cuentasPorPagar.data.filter(item => isOpen(item) && !isOverdue(item)).reduce((total, item) => total + Number(item.saldo), 0).toFixed(4),
  cantidadPendiente: props.cuentasPorPagar.data.filter(isOpen).length
})
const statusColor = (account: CuentaPorPagar): 'warning' | 'info' | 'success' | 'neutral' => {
  const state = accountState(account)
  return state === 'pendiente' ? 'warning' : state === 'parcial' ? 'info' : state === 'pagada' ? 'success' : 'neutral'
}
const statusLabel = (account: CuentaPorPagar) => ({ pendiente: 'Pendiente', parcial: 'Parcial', pagada: 'Pagada', anulada: 'Anulada' })[accountState(account)]
...
const registerDisbursement = (account: CuentaPorPagar) => router.visit(route('desembolsos.create', {
  edificio_id: account.edificioId,
  proveedor_id: account.proveedorId
}))
</script>
  ...
  <UDashboardPanel id="cuentas-por-pagar">
    ...
    <template #body>
      <div class="flex h-full flex-col gap-5 p-4 sm:p-6">
        <UAlert color="info" description="Vista de consulta generada por gastos a crédito. Los saldos se actualizan automáticamente al registrar o anular desembolsos." icon="i-lucide-info" variant="subtle" />
        ...
        <UCard><div class="grid gap-3 md:grid-cols-3"><USelect v-model="edificioId" :items="[{ label: 'Todos los edificios', value: '' }, ...edificios.map(item => ({ label: item.nombre, value: item.id }))]" @update:model-value="changeBuilding" /><USelect v-model="proveedorId" :items="[{ label: 'Todos los proveedores', value: '' }, ...providerItems]" @update:model-value="reload(1, true)" /><USelect v-model="estado" :items="[{ label: 'Todos los estados', value: '' }, { label: 'Pendiente', value: 'pendiente' }, { label: 'Parcial', value: 'parcial' }, { label: 'Pagada', value: 'pagada' }, { label: 'Anulada', value: 'anulada' }]" @update:model-value="reload(1, true)" /></div></UCard>
        <div class="overflow-x-auto rounded-xl border border-default" :class="loading ? 'opacity-60' : ''"><table class="w-full min-w-5xl text-left text-sm"><thead class="bg-elevated/50 text-xs uppercase tracking-wide text-muted"><tr><th class="p-3">Gasto</th><th class="p-3">Proveedor</th><th class="p-3">Vencimiento</th><th class="p-3 text-right">Monto original</th><th class="p-3 text-right">Saldo</th><th class="p-3">Estado</th><th class="p-3 text-right">Acciones</th></tr></thead><tbody class="divide-y divide-default"><tr v-for="cuenta in cuentasPorPagar.data" :key="cuenta.id" class="cursor-pointer hover:bg-elevated/30" @click="router.visit(route('cuentas-por-pagar.show', [cuenta.edificioId, cuenta.id]))"><td class="p-3"><p class="font-mono font-semibold text-primary">{{ cuenta.numeroGasto }}</p><p class="text-xs text-muted">{{ cuenta.edificio }}</p></td><td class="p-3 font-medium text-highlighted">{{ cuenta.proveedor }}</td><td class="p-3"><p :class="isOverdue(cuenta) ? 'font-medium text-error' : ''">{{ cuenta.fechaVencimiento }}</p><p v-if="isOverdue(cuenta)" class="text-xs text-error">{{ daysOverdue(cuenta.fechaVencimiento) }} día(s) vencida</p></td><td class="p-3 text-right font-mono">${{ cuenta.montoOriginal }}</td><td class="p-3 text-right font-mono font-semibold">${{ cuenta.saldo }}</td><td class="p-3"><UBadge :color="statusColor(cuenta)" :label="statusLabel(cuenta)" variant="subtle" /></td><td class="p-3"><div class="flex justify-end gap-1"><UButton v-if="canRegister(cuenta)" icon="i-lucide-banknote-arrow-down" label="Registrar" size="sm" variant="outline" @click.stop="registerDisbursement(cuenta)" /><UButton color="neutral" icon="i-lucide-eye" variant="ghost" aria-label="Ver cuenta por pagar" @click.stop="router.visit(route('cuentas-por-pagar.show', [cuenta.edificioId, cuenta.id]))" /></div></td></tr><tr v-if="!cuentasPorPagar.data.length"><td class="p-10 text-center text-muted" colspan="7">No hay cuentas por pagar que coincidan con los filtros.</td></tr></tbody></table></div>
