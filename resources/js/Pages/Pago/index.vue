...
const reload = (page = 1) =>
    router.get(
        route('pagos.index'),
        {
            ...
            propietario_id: propietarioId.value || undefined,
    ...
    <UDashboardPanel id="pagos"
        ><template #header
            ><UDashboardNavbar title="Pagos"
                ...
                ><template #right
                    ><UButton v-if="canAny('pagos.registrar')" icon="i-lucide-plus" label="Registrar pago" @click="router.visit(route('pagos.create'))" /></template></UDashboardNavbar></template
        ><template #body
            ><div class="flex h-full flex-col gap-5 p-4 sm:p-6">
                <div class="grid gap-3 xl:grid-cols-3">
                    ...
                    /><USelect v-model="departamentoId" :items="[{ label: 'Todos los departamentos', value: '' }, ...departments]" size="xl" @update:model-value="reload()" /><USelect
                        v-model="propietarioId"
                        :items="[
                            { label: 'Todos los propietarios', value: '' },
                    ...
                    /><UInput v-model="fechaDesde" type="date" size="xl" aria-label="Fecha desde" @change="reload()" /><UInput
                        v-model="fechaHasta"
                        type="date"
                        size="xl"
                    ...
                    <table class="w-full min-w-5xl text-left text-sm">
                        <thead class="bg-elevated/50 text-xs uppercase tracking-wide text-muted">
                            <tr>
                                ...
                                <th class="p-3 text-right">Aplicado</th>
                                <th class="p-3 text-right">Saldo a favor</th>
                                <th class="p-3">Forma</th>
                                <th class="p-3">Estado</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-default">
                            <tr v-for="pago in pagos.data" :key="pago.id" class="cursor-pointer hover:bg-elevated/30" @click="router.visit(route('pagos.show', [pago.edificioId, pago.id]))">
                                <td class="p-3 font-mono text-primary">
                                    {{ pago.numero }}
                                </td>
                                <td class="p-3">{{ pago.fechaPago }}</td>
                                <td class="p-3">{{ pago.departamento }}</td>
                                <td class="p-3">
                                    {{ pago.propietario || 'Sin titular único' }}
                                </td>
                                <td class="p-3 text-right font-mono">${{ pago.valorRecibido }}</td>
                                <td class="p-3 text-right font-mono">${{ pago.valorAplicado }}</td>
                                <td class="p-3 text-right font-mono">${{ pago.saldoFavor }}</td>
                            ...
                            <tr v-if="!pagos.data.length">
                                <td class="p-8 text-center text-muted" colspan="9">No hay pagos para los filtros seleccionados.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div v-if="pagos.meta.lastPage > 1" class="flex items-center justify-between text-sm text-muted">
                    <span>{{ pagos.meta.total }} pago(s)</span><UPagination :page="pagos.meta.currentPage" :total="pagos.meta.total" :items-per-page="pagos.meta.perPage" @update:page="reload" />
                </div></div></template
    ></UDashboardPanel>
</template>
